 ...
 export default function PilihMurid ({route , navigation, } : any) {
  ...
  const getdata = async () => {
    ...
    try{
      ...
      axios.get(`${BASE_URL_STAG}/kidsgbigama_api.api.students.api.list_student?page=1&per_page=50&class=${useee.id}&search=`, config)
        .then(response => {
          // Handle response
  ...
  const getkelas = async () => {
    ...
    try{
      ...
      const config = {
        ...
        axios.get(`${BASE_URL_STAG}/kidsgbigama_api.api.class.api.list`, config)
        .then(response => {
          // Handle response
          // console.log(response.data);
          // Alert.alert("Login Successful", "Welcome back!");
